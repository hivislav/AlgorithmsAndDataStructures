...
public class TreeImpl<E extends Comparable<E>> implements Tree<E> {
    ...
    public boolean add(E value) {
        ...
        if (nodeAndParent.level >= 4) { //максимальный уровень дерева
            return false;
        }
    ...
    public boolean isBalance() {
        int leftLevel = findLeftLevel(root.getLeftChild());
        int rightLevel = findRightLevel(root.getRightChild());
    ...
    private int findLeftLevel(Node<E> current) {
        if (current == null) {
            return level;
        }

        NodeAndParent nodeAndParent = doFind(current.getValue());

        if (nodeAndParent.level > level) {
            level = nodeAndParent.level;
        }

//        System.out.println("Уровень левого древа: " + (level - 1));

        findLeftLevel(current.getLeftChild());
        findLeftLevel(current.getRightChild());
        return level;
    }

    private int findRightLevel(Node<E> current) {
        if (current == null) {
            return level;
        }
...
//        System.out.println("Уровень правого древа: " + (level - 1));
